...
document.getElementById('cancel_btn_messageModifyClose').addEventListener('click', function(e) {
	e.preventDefault();
	overlay.style.display = 'none';
...
document.getElementById('chatroomDelClose').addEventListener('click', function(e) {
	e.preventDefault();
	overlay.style.display = 'none';
...
document.getElementById('chatroomOutClose').addEventListener('click', function(e) {
	e.preventDefault();
	overlay.style.display = 'none';
...
document.getElementById('createTaskClose').addEventListener('click', function(e) {
	e.preventDefault();
	wrapper.classList.toggle('show-aside');
	bookmarkTotalContainer.style.display = 'none';
	voteTotalContainer.style.display = 'none';
...
function showAlarm() {
	...
	const alarmIcon = document.querySelector('img[alt="i_alarm"]');
	const workspaceId = alarmIcon.dataset.workspaceId;

	if (link.style.display === 'block') {
		fetch(`/${workspaceId}/alarm/mark`)
			.then(response => {
				if (!response.ok) throw new Error("❌ 알람 읽음 처리 실패");

				console.log("🌿 알람 읽음 처리 완료");
			})
			.catch(error => {
				console.error("❌ 알람 읽음 처리 중 오류:", error);
			});
	}
}
